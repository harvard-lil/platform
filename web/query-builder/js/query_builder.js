...
var config = (function () { 
    ...
    my.request_location = 'http://librarycloud.harvard.edu/v1/api/';
    ...
    my.item_options = {
    		...
    		'lcsh_keyword' : 'Subject Heading Keyword',
    		'language': 'Language'
    };
...
//		rows += '<div class="form_element"><select id="search_type">' + option_string + '</select></div></div>';
		...
		rows += '<div class="form_element"><input type="text" id="query" value="internet"/></div></div>';
...
var exec_form = (function () { 
    ...
    make_request = function(request_string) {
		...
		$.ajax({
		  ...
		  error: function(data) {
  		  	$('#lc_response').val( 'Well sweet corn in the mornin\'! Something went wrong. Is your request malformed?');
  		  }
		});
    }
